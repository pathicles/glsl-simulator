import {ASTVisitor,ASTNode_Types} from "./visitor.mjs";
import Builtins from "../runtime/builtins.mjs";
import Operations from "../runtime/ops.mjs";
...
class CodeGenerator extends ASTVisitor {
    constructor(shader)
    {
        ...
        for (let list = shader.uniforms, i = 0; i < list.length; ++i)
    ...
    visitorForType(type)
    {
        ...
        return super.visitorForType(type);
    }

    // Private
    _resolveGetLocal(name)
    {
    ...
    _resolveSetName(name, valueExpr)
    {
        console.assert(typeof name === "string", name);
